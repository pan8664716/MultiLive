...
- 播放地址形态：**全部平台不写直链**，m3u 统一写 `https://astar.cc.cd/<平台>/<房间号>`（douyin/yy/bilibili/douyu/huya/kuaishou），由 Worker 点播时实时解析（看哪个解析哪个）
...
1. **绝不逐房间调 API 获取信息或播放地址**（批量请求会触发风控）。只能：
   ...
   - 播放地址：统一写 `https://astar.cc.cd/<平台>/<房间号>`，由 Worker 点播时解析；
